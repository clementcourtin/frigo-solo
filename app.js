...
function render() {
  ...
  for (const item of visible) {
    ...
    row.addEventListener('touchmove', (event) => { const t = event.changedTouches[0], x = t.clientX - swipeStartX, y = t.clientY - swipeStartY; if (Math.abs(x) > Math.abs(y)) { const strength = Math.min(Math.abs(x) / 140, 1); row.style.transform = 'translateX(' + x + 'px) rotate(' + (x / 42) + 'deg)'; row.style.opacity = String(1 - Math.min(Math.abs(x) / 900, .3)); row.style.background = x < 0 ? 'rgba(211, 87, 76, ' + (strength * .28) + ')' : 'rgba(76, 151, 97, ' + (strength * .28) + ')'; row.style.boxShadow = x < 0 ? '-8px 4px 20px rgba(211, 87, 76, ' + (strength * .28) + ')' : '8px 4px 20px rgba(76, 151, 97, ' + (strength * .28) + ')'; } }, { passive: true });
    row.addEventListener('touchend', (event) => { const t = event.changedTouches[0], x = t.clientX - swipeStartX, y = t.clientY - swipeStartY; row.style.transition = 'transform 220ms cubic-bezier(.2,.8,.2,1), opacity 180ms ease, background 180ms ease, box-shadow 180ms ease'; if (Math.abs(x) < 72 || Math.abs(x) < Math.abs(y)) { row.style.transform = ''; row.style.opacity = ''; row.style.background = ''; row.style.boxShadow = ''; return; } row.style.transform = 'translateX(' + (x < 0 ? '-110%' : '110%') + ') rotate(' + (x < 0 ? '-10deg' : '10deg') + ')'; setTimeout(() => { if (x < 0) consumeFood(item.id); else { addToShopping(item.name); row.style.transform = ''; row.style.opacity = ''; row.style.background = ''; row.style.boxShadow = ''; } }, 220); }, { passive: true });
    foods.append(node);
  }
}
